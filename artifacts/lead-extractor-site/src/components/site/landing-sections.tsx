...
// Trustpilot star SVG
const TpStar = ({ filled = true }: { filled?: boolean }) => (
  <div className={`w-[22px] h-[22px] flex items-center justify-center ${filled ? "bg-[#00b67a]" : "bg-[#dcdce6]"}`}>
...
// Avatar initials circle with Google-style colors
const AVATAR_COLORS = [
  { bg: "#4285F4", text: "#fff" },
  { bg: "#EA4335", text: "#fff" },
  { bg: "#34A853", text: "#fff" },
...
const PLATFORM_REVIEWS = [
  { platform: "google" as const, name: "Sarah M.", business: "Digital Marketing Agency", quote: "Ordered 200 roofing leads for three different cities. Every single one had a working phone number. Closed four jobs in the first week — best $58 I've ever spent on marketing.", date: "2 weeks ago" },
  { platform: "trustpilot" as const, name: "Marcus D.", business: "Web Design Freelancer", quote: "I pitch website redesigns to local businesses. These lists tell me exactly which ones have no site or a broken one. My close rate went from 12% to 31% because I'm only calling people who already need me.", date: "a month ago" },
  { platform: "google" as const, name: "Rachel T.", business: "SEO Consultant", quote: "The CSV is clean and ready to drop straight into my CRM. No scrubbing, no deduplication headaches. The \"few reviews\" filter is gold — those owners are desperate and pick up fast.", date: "3 weeks ago" },
  { platform: "trustpilot" as const, name: "Chen W.", business: "Insurance Broker", quote: "Tried three other lead providers before this. None of them verified the data. Map Lead Extractor actually checks the numbers — I wasted half my day on dead lines with the others.", date: "2 months ago" },
  { platform: "google" as const, name: "Priya M.", business: "Reputation Management", quote: "I buy the \"low rating\" filter every month. Restaurants and salons under 4 stars are my bread and butter — owners are embarrassed and motivated. Pack pays for itself in one call.", date: "6 weeks ago" },
  { platform: "trustpilot" as const, name: "James O.", business: "Solar Sales", quote: "Got 150 leads for small commercial properties in my region. Delivery was faster than promised and the refund process for the handful of bad numbers was instant. Will order again.", date: "a month ago" },
];

function GoogleReviewCard({ review, colorIdx }: { review: typeof PLATFORM_REVIEWS[0]; colorIdx: number }) {
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0" style={{ backgroundColor: color.bg, color: color.text }}>
            {review.name[0]}
          </div>
          <div>
      ...
      <div className="flex gap-0.5 mb-2">
        {[1,2,3,4,5].map(s => (
          <svg key={s} className="w-4 h-4 fill-[#fbbc04]" viewBox="0 0 20 20">
            <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/>
          </svg>
        ))}
      </div>
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex gap-0.5">
          {[1,2,3,4,5].map(s => <TpStar key={s} />)}
        </div>
        {/* Trustpilot logo */}
        <div className="flex items-center gap-1">
          <svg width="16" height="16" viewBox="0 0 126.3 125.5"><path d="M126.3 48.2H78L63.2 2.5 48.3 48.2H0l40.5 29.1-15.4 47 38.1-27.5 38.2 27.5-15.5-47z" fill="#00b67a"/></svg>
          <span className="text-[11px] font-bold text-[#191919] tracking-tight">Trustpilot</span>
        </div>
      </div>
      {/* Verified badge */}
    ...
    <section className="py-20 border-t border-border">
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visible.map((r, i) =>
            r.platform === "google"
              ? <GoogleReviewCard key={r.name} review={r} colorIdx={i} />
        ...
        <div className="flex justify-center mt-8">
          ...
          >
            {showAll
              ...
              : <>Read all 6 reviews <ChevronDown className="w-4 h-4 transition-transform" /></>
            }
          </button>
        </div>
      </div>
