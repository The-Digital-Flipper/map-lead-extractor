              ...
              geographies.map((geo) => {
                ...
                return (
                  <Geography
                    ...
                    onMouseEnter={() => {
                      setTooltipContent(
                        `${STATE_NAMES[abbr] ?? abbr}: ${count.toLocaleString()} lead${count !== 1 ? "s" : ""}`
                      );
                    }}
                    onMouseLeave={() => setTooltipContent("")}
                    style={{
                      default: { outline: "none", cursor: "pointer" },
                      hover: { outline: "none", fill: "#00E676", opacity: 0.85 },
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.05 }}
            ...
            {[
              ...
              { label: "Total Users", value: "—", icon: <Users className="w-4 h-4 text-blue-400" />, sub: "via Clerk" },
              ...
              {geo ? (
                Object.keys(geo.byState).length === 0 ? (
                  <div className="py-16 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">No US address data yet — leads will appear here once addresses are extracted.</p>
                  </div>
                ) : (
              ...
              {topStates.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={topStates} layout="vertical" margin={{ left: 0, right: 24 }}>
                    ...
                    <RechartsTooltip
                      contentStyle={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 8, color: "#e6edf3" }}
                      formatter={(v: number) => [v.toLocaleString(), "Leads"]}
                    />
                    <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                      {topStates.map((_, i) => (
                        <Cell key={i} fill={i === 0 ? "#00E676" : `rgba(0,230,118,${0.85 - i * 0.08})`} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              {loadingLeads ? (
                ...
                <div className="py-20 text-center">
                  ...
                  <p className="text-sm text-muted-foreground max-w-sm mx-auto">
                    Once users install the extension and extract leads, they'll appear here in real time.
                  </p>
                </div>
              ) : (
                <>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {leads.map((lead, i) => (
                          <tr key={lead.id}
                            className={`border-b border-border/50 hover:bg-white/[0.02] transition-colors ${i % 2 !== 0 ? "bg-white/[0.01]" : ""}`}>
