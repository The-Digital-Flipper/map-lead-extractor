...
async function handleLicenseFulfillment(payload: Buffer, signature: string): Promise<void> {
  ...
  // Expand line items to confirm this is the $197 license product
  const stripe = await getUncachableStripeClient();
  let isLicense = false;
  try {
    ...
    isLicense = (full.line_items?.data ?? []).some((item) => {
    ...
    logger.error({ err, sessionId: session.id }, 'Could not expand session line items for license check');
    return;
  }
