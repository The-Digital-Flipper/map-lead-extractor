...
const els = {
  ...
  copyEmails: document.querySelector("[data-copy-emails]"),
  copyPhones: document.querySelector("[data-copy-phones]"),
  emailAll: document.querySelector("[data-email-all]"),
...
const HELP_CONTENT = {
  how: `<h3>How it works</h3>
    <ol>
      <li>Open <b>Bing Maps</b> and search a business type and area — e.g. <i>food in Dallas</i>.</li>
    ...
    <p><b>Bing only shows a limited number of businesses per map view</b> (usually ~100–200). Once a run
    finishes, that view is tapped out — scrolling more won't add new ones.</p>
    <p>To pull more leads, <b>pan or zoom the map to a different area</b> and run again, or click Bing's
...
async function init() {
  ...
  wireEvents();
  setStatus("Ready. Open Bing Maps search results, then start extraction.", "idle");
  render();
  reportPanelHeight();
}
...
function wireEvents() {
  ...
  els.copyEmails.addEventListener("click", async () => {
    const list = collectValues("Emails");
    if (!list.length) { setStatus("No emails yet — turn on enrichment to collect them.", "warn"); render(); return; }
    await copyText(list.join(", "));
    setStatus(`Copied ${list.length} email(s) to your clipboard.`, "complete");
    render();
  });

  els.copyPhones.addEventListener("click", async () => {
    const list = collectValues("Phone");
    if (!list.length) { setStatus("No phone numbers collected yet.", "warn"); render(); return; }
    await copyText(list.join(", "));
    setStatus(`Copied ${list.length} phone number(s) to your clipboard.`, "complete");
    render();
  });

  els.emailAll.addEventListener("click", () => {
    const list = collectValues("Emails");
    if (!list.length) { setStatus("No emails yet — turn on enrichment to collect them.", "warn"); render(); return; }
    window.open(`mailto:?bcc=${encodeURIComponent(list.join(","))}`, "_blank");
    setStatus(`Opening an email to ${list.length} lead(s) (BCC).`, "complete");
    render();
  });
...
async function runHarvest() {
  ...
  lastLayoutName = "";
  setStatus("Finding Bing Maps result list...", "running");
  render();
  ...
  if (!initialWait?.ok) {
    setStatus(initialWait?.error || "Bing layout not recognized - results may have changed.", "warn");
  }
  ...
  while (!stopRequested) {
    ...
    if (!wait?.ok) {
      setStatus(wait?.error || "Bing layout not recognized - results may have changed.", "warn");
    }
  }
...
async function harvestCurrentPage(stableAttempts) {
  ...
  while (!stopRequested) {
    ...
    if (!harvest?.ok) {
      return { ok: false, message: harvest?.error || "Could not read visible Bing Maps results." };
    }
    ...
    setStatus(`Harvesting - ${loadedVisibleCount} loaded / ${rowsByKey.size} total found.`, "running");
    render();
    ...
    if (!scroll?.ok) {
      return { ok: false, message: scroll?.error || "Could not scroll the Bing Maps result list." };
    }
...
function entityToRow(wrapped) {
  ...
  return {
    ...
    Name: name,
    Address: address,
    "Bing Maps URL": buildBingMapsUrl(name, address, latitude, longitude),
    Rating: firstValue(
      entity.rating,
...
function buildBingMapsUrl(name, address, latitude, longitude) {
  const query = encodeURIComponent([name, address].filter(Boolean).join(" "));
  if (latitude && longitude) {
    return `https://www.bing.com/maps?cp=${latitude}~${longitude}&lvl=16.0&q=${query}`;
  }
  if (query) {
...
// Collect unique values from a column across all rows (splits comma lists).
...
function render() {
  ...
  els.copyEmails.disabled = rowsByKey.size === 0;
  els.copyPhones.disabled = rowsByKey.size === 0;
  els.emailAll.disabled = rowsByKey.size === 0;
...
function fillIconCell(td, header, value) {
  ...
  if (header === "Bing Maps URL") {
    td.append(makeLogoLink(value, "📍", "Open in Bing Maps"));
    return;
  }
