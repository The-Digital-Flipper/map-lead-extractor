...
- **Free sample leads (proof-first email capture)**: the pack widget (`lead-extractor-site/src/components/site/lead-pack-widget.tsx`) has a "See 5 real leads free" block. `POST /api/stripe/pack-sample` (in `api-server/src/routes/stripe.ts`) takes a `{category,state}` or free-text `{request}`, returns 5 REAL matching leads — name, city, rating, website, **phone masked to the area code** (`(601) •••-••••`), email hidden — ordered to surface the most *complete* records (has email+website+rating) so the preview looks impressive (the actual pack still ships top-100 by `value_score`). It records the view in the new `sample_requests` table (`lib/db/src/schema/sampleRequests.ts`) with `email` null. Entering an email calls `POST /api/stripe/pack-sample-unlock` `{sampleId,email}`, which stamps `email`+`unlocked_at` on that row and returns the SAME 5 leads with full phone + first email revealed, then shows a "get all 100 for $29" CTA. So `sample_requests WHERE email IS NOT NULL` is the captured lead list; the whole table is the sample-flow funnel (viewed vs. unlocked). Both sample endpoints are public/no-auth. Table + columns were created with additive SQL (`CREATE TABLE IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS`, never drizzle push — see Gotchas). **Admin "✉️ Captured Leads" tab** (`admin.tsx`, endpoints `GET /api/admin/captured-leads`, `GET /api/admin/captured-leads/export.csv`, `POST /api/admin/captured-leads/:id/follow-up`, all `requireAuth`) lists captures with funnel stats (views/captures/followed-up/purchased/unsubscribed), CSV export, and a manual "Send follow-up" button. **Buyer follow-up nurture** (`api-server/src/lib/buyer-followup.ts`, scheduler started in `index.ts`): ~1h after unlock, auto-sends ONE "here are your 5 samples, get the other 95 for $29" email to captures that haven't bought (matched on `pack_orders.email`), opted out, or been sent already. It is deliberately SEPARATE from the cold-outreach engine (that engine is keyed to `leads` rows; these recipients are prospective customers) — it only reuses the low-level Gmail/Resend sender + sender identity from `outreach-auto.ts`. Gated on a configured provider (`providerReady`); compliant with a one-click List-Unsubscribe header + footer link backed by public `GET/POST /api/stripe/sample-unsub/:token` (sets `unsubscribed_at`, hard-stops the nurture) and the owner's `business_address` footer.
...
- Social auto-poster: `api-server/src/lib/social.ts` owns AI post generation (OpenAI key), Facebook publishing, and a 5-min scheduler started in `index.ts` (1 post/day at `social_settings.post_hour_utc`, auto-refills the queue below 3). **Two ad campaigns share the one-per-day Page slot, tagged by `social_posts.campaign`: `leads` (default — sells the paid done-for-you lists, generated by `generateSocialPosts`) and `freetool` (promotes the FREE Chrome extension with its Web Store install link in the body, generated by `generateFreeToolPosts`). The scheduler rotates ~1 freetool post for every 2 leads posts, keyed off `postedCount % 3 === 2`, falling back to the other campaign when the preferred queue is empty; each campaign auto-refills independently (leads < 3, freetool < 2). Admin Social tab has separate "Generate 5 posts" / "Free-tool posts" buttons and a 🧩 badge on freetool rows; POST `/api/admin/social/generate-freetool` writes them. Admin **📣 Social** tab drives it via `/api/admin/social*`. Facebook connects via the tab's one-click OAuth button (`/api/admin/social/fb/connect` → callback stores the long-lived Page token in `social_settings`; app id/secret are seeded in that same DB row, env `FACEBOOK_*` vars are a fallback). The FB app (id 1073926301971765) needs the callback URL whitelisted under Facebook Login → Valid OAuth Redirect URIs.
